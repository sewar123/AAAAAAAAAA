...
bot = telebot.TeleBot("7692526711:AAED6-sUtN9N8qt8Nd6VYlOPKxQcHdG51R4")
bot.remove_webhook()
...
api_id = 23767639
api_hash = "2fbab2f206df295b25380fd5f9223635"
...
# تعديل 2 
@bot.message_handler(func=lambda message: message.text == "بيمو")
def withdraw_balance(message):
    bot.send_message(message.chat.id, "غير متوفر حاليا..اختر طريقة شحن اخرة او تواصل مع الدعم الفني ")


@bot.message_handler(func=lambda message: message.text == "بايير")
def withdraw_balance(message):
    bot.send_message(message.chat.id, "غير متوفر حاليا..اختر طريقة شحن اخرة او تواصل مع الدعم الفني ")
